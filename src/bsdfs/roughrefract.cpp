...
class RoughRefract final : public BSDF<Float, Spectrum> {
public:
    MI_IMPORT_BASE(BSDF, m_flags, m_components)
    MI_IMPORT_TYPES(Texture, MicrofacetDistribution)

    RoughRefract(const Properties &props) : Base(props) {
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
        ...
        bool has_reflection = false, has_transmission = true;
        ...
        if (unlikely(has_reflection && has_transmission)) {
    ...
    Float pdf(const BSDFContext &ctx, const SurfaceInteraction3f &si,
        ...
        if (unlikely(has_transmission && has_reflection)) {
            Float F = std::get<0>(fresnel(dr::dot(si.wi, m), m_eta));
...
MI_IMPLEMENT_CLASS_VARIANT(RoughRefract, BSDF)
MI_EXPORT_PLUGIN(RoughRefract, "Rough Refract")
