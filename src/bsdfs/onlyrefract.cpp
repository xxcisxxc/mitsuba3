...
class OnlyRefract final : public BSDF<Float, Spectrum> {
public:
    MI_IMPORT_BASE(BSDF, m_flags, m_components)
    MI_IMPORT_TYPES(Texture)

    OnlyRefract(const Properties &props) : Base(props) {
    ...
    std::pair<BSDFSample3f, Spectrum> sample(const BSDFContext &ctx,
        ...
        Mask selected_r = false && active;  // Always false
...
MI_IMPLEMENT_CLASS_VARIANT(OnlyRefract, BSDF)
MI_EXPORT_PLUGIN(OnlyRefract, "Only Refract")
